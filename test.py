...
pagelist = ["（人の能力）同一人物の同一行程でのばらつきの把握_ヒストグラム","（人の能力）同一行程内のばらつき把握_ヒストグラム","（人の能力）作業時間統計量","（人の能力）各人の工程量",#人の能力
            "(A-1)各人各日の実績ガントチャート","（A-2）各工程各日の実績ガントチャート",#場所・時間・人の有効活用
            "（D）一つの製品の総社内滞在時間の把握"]#無駄なものを作りたくない
...
#=======================================================================================================================================================
elif selector=="（人の能力）同一行程内のばらつき把握_ヒストグラム":
    st.session_state.df['開始日時']=pd.to_datetime(st.session_state.df['開始日時'])
    #曜日の設定
    st.session_state.df["曜日"]=st.session_state.df["工程開始日"].dt.weekday
    #月の設定
