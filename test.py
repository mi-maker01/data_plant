...
#セレクトボックスのリストを作成
pagelist = ["(A-1)各人各日の実績ガントチャート","（A-2）各工程各日の実績ガントチャート","（B）同一人物の同一行程でのばらつきの把握_ヒストグラム","（C）同一行程内のばらつき把握_ヒストグラム","（D）一つの製品の総社内滞在時間の把握","（E）担当者別作業時間統計量","（E）図番別作業時間統計量","（E）作業時間統計量","（E）各人の工程量"]
st.title("生産データ分析")
#サイドバーのセレクトボックスを配置
    ...
    if answer == True:
        ...
        for z in z_list:#図番でfor文回す
            for k in k_list:#工程名称でfor文回す
                ...
                pvit=pd.DataFrame(dd.agg(["count","mean","median","min", "max"]))
                pvit=pvit.set_axis(['件数', '平均', '中央値', '最小', '最大'], axis=1)
                st.write(pvit.T)
#=======================================================================================================================================================
