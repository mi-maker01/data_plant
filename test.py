...
#セレクトボックスのリストを作成
pagelist = ["➀（人の能力）同一人物の同一行程でのばらつきの把握_ヒストグラム","➀(ヒストグラム)作業時間のばらつき","➀(集計表)作業時間統計量","➀(棒グラフ)期間内の各人作業量",#人の能力
            "➁(ガントチャート)各人各日の実績","➁(ガントチャート)各工程各日の実績",#場所・時間・人の有効活用
            "➂(折れ線グラフ)仕掛品の把握"]#無駄なものを作りたくない
...
elif selector=="➀（人の能力）同一人物の同一行程でのばらつきの把握_ヒストグラム":
...
#             for k in k_num:#工程名称でfor文回す
        ...
        if f_num=="なし":
            ...
            for nen in nen_list:
                ...
                ax.hist(dd,bins=10,range=(lower_num2,upper_num2))
                # Matplotlib の Figure を指定して可視化する
                st.write("---------------工程コード:",k,"-------------図番:",z,"------------データの数:",len(scores),"-------年:",nen,"--------------")
                left_column, right_column = st.columns(2)
                left_column.pyplot(fig)
...
#=======================================================================================================================================================
elif selector=="➀(ヒストグラム)作業時間のばらつき":
    st.session_state.df['開始日時']=pd.to_datetime(st.session_state.df['開始日時'])
    #曜日の設定
    st.session_state.df["曜日"]=st.session_state.df["工程開始日"].dt.weekday
    #月の設定
